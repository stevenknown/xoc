...
int main(int argc, char *argv[]) {
    ...
    for (int step = 0; step < 10; step++) {
        ...
        double time_elapsed_s = (end.tv_sec - start.tv_sec) + (end.tv_nsec - start.tv_nsec) / 1000000000;
        ...
        if (step == 0) {
            ...
            for (int i=0; i<B*T*V; i++) {
                ...
                if (fabs(expected_logits[i] - model.acts.logits[i]) >= 0.01) {
            ...
            if (fabs(model.mean_loss - *expected_loss) >= 0.01) {
        ...
        gpt2_update(&model, 0.0001, 0.9f, 0.999f, 0.00000001, 0.01f, step+1);
    ...
    // compare
    for (int i = 0; i < 10; i++) {
        if (fabs(losses[i] - expected_losses[i]) >= 0.01) {
